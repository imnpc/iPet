...
@auth
    ...
    <form action="{{ route('posts.comments.like', ['post' => $post, 'comment' => $comment]) }}" method="POST">
        ...
        <button
            type="submit"
            @disabled($isLiked)
            class="inline-flex items-center gap-1 rounded-full border px-2.5 py-1 font-medium transition {{ $isLiked ? 'cursor-not-allowed border-primary-200 bg-primary-50 text-primary-600' : 'border-warm-200 text-warm-600 hover:border-primary-300 hover:text-primary-600' }}"
        >
